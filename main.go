...
func main() {
	var (
		err       error
		gameInfo  GameInfo
		mainBoxer IsOpponent
	)

	isHost, ip := GetConnectionInfo()
	ctx, cancelFunc := context.WithCancel(context.Background())
	gameInfo.Cancel = cancelFunc

	if isHost {
		// Initialize tcp connection (room) on server
		Conn, err = StartServer(7777)
		if err != nil {
			clearScreen()
			cancelFunc()
		}
	} else {
		Conn, err = Dial(ip, 7777)
		if err != nil {
			fmt.Println(err)
			frame(100)
			clearScreen()
	...
	// Initialize channels
	//mainInput := make(chan Act)
	//oppInput := make(chan Act)

	// Read keyboard for first player
	go ReadKeyboard(MainChan, isHost, Conn, cancelFunc)

	// Read second player event
	//go ReadSecondPlayerEvent(oppInput, Conn)
	go Conn.Listen(OppChan)

	// Main boxer
	go func() {
		for act := range MainChan {
			switch act {
			case DoPunch:
